...
from app.services.bank_immo_import import (
    _ImportCandidate,
    apply_excel_reclassification,
    apply_excel_sortie,
    compute_dotation_exercice,
    find_unique_exit_match,
    find_unique_reclassification_match,
    parse_amount,
    parse_bank_workbook,
    parse_date,
    parse_taux,
...
def test_find_unique_exit_match_by_vb_and_cumul():
    cat_id = __import__("uuid").uuid4()
    kept = Immobilisation(
        code_inventaire="AAI-2019-006",
        designation="GAB SefServ27(S2)",
        quantite=2,
        date_acquisition=date(2019, 6, 18),
        valeur_brute=Decimal("1129260.00"),
        statut=StatutImmobilisation.EN_SERVICE,
        categorie_id=cat_id,
    )
    exited = Immobilisation(
        code_inventaire="AAI-2019-007",
        designation="GAB SefServ27(S2)",
        quantite=2,
        date_acquisition=date(2019, 6, 18),
        valeur_brute=Decimal("1129260.00"),
    ...
    candidates = [
        _ImportCandidate(immo=kept, amt_n1=Decimal("621093.00")),
        _ImportCandidate(immo=exited, amt_n1=Decimal("56463.00")),
    ]
    match = find_unique_exit_match(
        candidates,
        categorie_id=cat_id,
        valeur_brute=Decimal("-1129260.00"),
        amt_n1=Decimal("-56463.00"),
        quantite=2,
        date_sortie=date(2020, 1, 9),
    )
    assert match is not None
    assert match.immo.code_inventaire == "AAI-2019-007"

    from app.services.bank_immo_import import ParsedBankRow
    ...
    apply_excel_sortie(
        ...
        source_row=ParsedBankRow(
            sheet="aai",
            categorie_code="TY-142010",
            row_number=225,
            date_acquisition=date(2020, 1, 9),
            quantite=2,
            designation="GAB Multifonction génération Outdoor Dielbold",
            valeur_brute=Decimal("-1129260.00"),
            taux=Decimal("10"),
            amt_n1=Decimal("-56463.00"),
            dotation=Decimal("0"),
            amt_fin=Decimal("-56463.00"),
            vnc=Decimal("-1072797.00"),
            agence_label=None,
            is_negative=True,
        ),
    )
    assert match.immo.statut == StatutImmobilisation.SORTIE
    assert match.immo.date_fin == date(2020, 1, 9)

    # Sorti en 2020 → absent du rapport 2026
    assert _mouvements_immo(match.immo, 2026) is None


def test_partial_negative_reclassification_keeps_and_amortizes_difference():
    cat_id = __import__("uuid").uuid4()
    construction = Immobilisation(
        code_inventaire="Const-2024-004",
        designation="DIFFERENT LOTS",
        quantite=1,
        date_acquisition=date(2024, 12, 31),
        valeur_brute=Decimal("352707000.00"),
        taux=Decimal("4"),
        statut=StatutImmobilisation.EN_SERVICE,
        categorie_id=cat_id,
    )
    candidates = [
        _ImportCandidate(immo=construction, amt_n1=Decimal("0.00"))
    ]
    match = find_unique_reclassification_match(
        candidates,
        categorie_id=cat_id,
        valeur_brute=Decimal("-342957000.00"),
        amt_n1=Decimal("0.00"),
        quantite=1,
        date_reclassement=date(2025, 10, 23),
    )
    assert match is not None
    ...
    valeur_apres = apply_excel_reclassification(
        ...
        source_row=ParsedBankRow(
            sheet="Construction",
            categorie_code="TY-142020",
            row_number=1,
            date_acquisition=date(2025, 10, 23),
            quantite=1,
            designation="REGULARISATION ECRITURE REF 7542039",
            valeur_brute=Decimal("-342957000.00"),
            taux=Decimal("4"),
            amt_n1=Decimal("0.00"),
            dotation=Decimal("0.00"),
            amt_fin=Decimal("0.00"),
            vnc=Decimal("-342957000.00"),
            agence_label=None,
            is_negative=True,
        ),
    )

    assert valeur_apres == Decimal("9750000.00")
    assert match.immo.valeur_brute == Decimal("9750000.00")
    assert match.immo.statut == StatutImmobilisation.EN_SERVICE
