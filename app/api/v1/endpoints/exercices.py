...
from app.schemas.exercice import (
    ...
    PeriodeAmortissementCategorieRead,
...
async def get_periodes_amortissement(
    ...
    try:
        service = PeriodeAmortissementService(db)
        rows = await service.list_periodes(annee)
        category_statuses = await service.category_statuses({p.id for p in rows})
        return [
            PeriodeAmortissementRead(
                ...
                categories=[
                    PeriodeAmortissementCategorieRead(
                        ...
                        statut=(
                            detail.statut.value
                            if hasattr(detail.statut, "value")
                            else str(detail.statut)
                        ),
                    )
                    for detail in category_statuses.get(p.id, [])
                ],
            )
            for p in rows
        ]
