...
class BankImportResult:
    ...
    reports_created: int
    negatives: int
    sorties_appliquees: int = 0
    reclassements_appliques: int = 0
...
class _ImportCandidate:
    immo: Immobilisation
    amt_n1: Decimal
    used_as_exit: bool = False


def find_unique_exit_match(
    ...
    *,
    ...
    quantite: int,
    date_sortie: date,
) -> _ImportCandidate | None:
    """Associe une ligne Excel négative à l'acquisition positive qu'elle annule."""
    abs_vb = abs(_q(valeur_brute))
    abs_cumul = abs(_q(amt_n1))
    matches = [
        candidate
        for candidate in candidates
        if not candidate.used_as_exit
        and candidate.immo.categorie_id == categorie_id
        and candidate.immo.date_fin is None
        and candidate.immo.statut == StatutImmobilisation.EN_SERVICE
        and candidate.immo.date_acquisition is not None
        and candidate.immo.date_acquisition <= date_sortie
        and _q(candidate.immo.valeur_brute) == abs_vb
        and _q(candidate.amt_n1) == abs_cumul
        and (quantite <= 0 or int(candidate.immo.quantite or 0) == quantite)
    ]
    if len(matches) == 1:
        return matches[0]
    return None
...
def find_unique_reclassification_match(
    candidates: list[_ImportCandidate],
    *,
    categorie_id: UUID,
    valeur_brute: Decimal,
    ...
    """Trouve l'actif positif unique réduit par une régularisation négative."""
    abs_vb = abs(_q(valeur_brute))
    abs_cumul = abs(_q(amt_n1))
    matches = [
        candidate
        ...
        and candidate.immo.statut == StatutImmobilisation.EN_SERVICE
        and candidate.immo.date_acquisition is not None
        and candidate.immo.date_acquisition < date_reclassement
        and _q(candidate.immo.valeur_brute) > abs_vb
        and _q(candidate.amt_n1) == abs_cumul
        and (quantite <= 0 or int(candidate.immo.quantite or 0) == quantite)
    ]
...
def apply_excel_sortie(
    immo: Immobilisation, *, date_sortie: date, source_row: ParsedBankRow
) -> None:
    """Marque une immobilisation comme sortie suite à une ligne Excel négative."""
    immo.statut = StatutImmobilisation.SORTIE
    immo.date_fin = date_sortie
    meta = dict(immo.metadata_json or {})
    meta["sortie_excel"] = {
        "date": date_sortie.isoformat(),
        "feuille": source_row.sheet,
        "ligne": source_row.row_number,
        "designation_source": source_row.designation,
        "valeur_brute": str(source_row.valeur_brute),
        "amt_n1": str(source_row.amt_n1),
    }
    immo.metadata_json = meta


def apply_excel_reclassification(
    immo: Immobilisation,
    *,
    date_reclassement: date,
    source_row: ParsedBankRow,
) -> Decimal:
    """Réduit la valeur brute d'un actif du montant négatif reclassé."""
    valeur_avant = _q(immo.valeur_brute)
    valeur_apres = _q(valeur_avant + source_row.valeur_brute)
    if valeur_apres <= 0:
        raise ValidationError("Le reclassement doit laisser une valeur brute positive.")
    immo.valeur_brute = valeur_apres
    meta = dict(immo.metadata_json or {})
    meta["reclassement_excel"] = {
        "date": date_reclassement.isoformat(),
        "feuille": source_row.sheet,
        "ligne": source_row.row_number,
        "designation_source": source_row.designation,
        "valeur_avant": str(valeur_avant),
        "montant_reclasse": str(source_row.valeur_brute),
        "valeur_apres": str(valeur_apres),
    }
    immo.metadata_json = meta
    return valeur_apres
...
class BankImmoImportService:
    ...
    async def import_from_bytes(self, content: bytes, filename: str = "import.xls") -> BankImportResult:
        ...
        reports_created = 0
        negatives = 0
        sorties_appliquees = 0
        reclassements_appliques = 0
        candidates: list[_ImportCandidate] = []
        ...
        for row in parsed:
            ...
            # Ligne Excel négative = sortie de l'acquisition positive correspondante.
            if row.is_negative or row.valeur_brute < 0:
                match = find_unique_exit_match(
                    candidates,
                    categorie_id=categorie.id,
                    valeur_brute=row.valeur_brute,
                    amt_n1=row.amt_n1,
                    quantite=row.quantite,
                    date_sortie=row.date_acquisition,
                )
                if match is not None:
                    ...
                    match.used_as_exit = True
                    sorties_appliquees += 1
                    compte = (
                        match.immo.compte_immobilisation
                        or categorie.compte_immobilisation
                        or ""
                    ).strip()
                    bucket = totaux.setdefault(
                        compte, BankImportTotaux(compte=compte)
                    )
                    bucket.valeur_brute = _q(
                        bucket.valeur_brute + row.valeur_brute
                    )
                    bucket.amt_fin = _q(bucket.amt_fin + row.amt_fin)
                    bucket.vnc = _q(bucket.vnc + row.vnc)
                    continue
                reclassification = find_unique_reclassification_match(
                    candidates,
                    categorie_id=categorie.id,
                    valeur_brute=row.valeur_brute,
                    amt_n1=row.amt_n1,
                    quantite=row.quantite,
                    date_reclassement=row.date_acquisition,
                )
                if reclassification is not None:
                    valeur_apres = apply_excel_reclassification(
                        reclassification.immo,
                        date_reclassement=row.date_acquisition,
                        source_row=row,
                    )
                    seeded = await self.db.execute(
                        select(Amortissement).where(
                            Amortissement.immobilisation_id
                            == reclassification.immo.id,
                            Amortissement.annule.is_(False),
                        )
                    )
                    for amort in seeded.scalars().all():
                        amort.vnc = max(
                            Decimal("0.00"),
                            _q(valeur_apres - amort.cumul),
                        )
                    reclassification.used_as_exit = True
                    reclassements_appliques += 1

                    compte = (
                        reclassification.immo.compte_immobilisation
                        or categorie.compte_immobilisation
                        or ""
                    ).strip()
                    bucket = totaux.setdefault(
                    ...
                    bucket.vnc = _q(bucket.vnc + row.vnc)
                    continue
                # Pas de rapprochement unique : conserver la ligne négative.
            ...
            try:
                ...
                if row.is_report:
                    reports_created += 1
                if row.valeur_brute > 0:
                    candidates.append(_ImportCandidate(immo=immo, amt_n1=_q(row.amt_n1)))
        ...
        return BankImportResult(
            ...
            negatives=negatives,
            sorties_appliquees=sorties_appliquees,
            reclassements_appliques=reclassements_appliques,
        )
