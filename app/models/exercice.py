...
class PeriodeAmortissementCategorie(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    """Validation trimestrielle d'une catégorie d'immobilisations."""

    __tablename__ = "periodes_amortissement_categories"
    __table_args__ = (
        UniqueConstraint(
            "periode_id",
            "categorie_id",
            name="uq_periode_amort_categorie",
        ),
    )

    periode_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("periodes_amortissement.id", ondelete="CASCADE"),
        index=True,
    )
    categorie_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("categories_immobilisation.id", ondelete="CASCADE"),
        index=True,
    )
    statut: Mapped[StatutPeriodeAmortissement] = mapped_column(
        Enum(
            StatutPeriodeAmortissement,
            name="statutperiodeamortissementcategorie",
            native_enum=False,
            values_callable=lambda enum_cls: [m.value for m in enum_cls],
        ),
        default=StatutPeriodeAmortissement.EN_ATTENTE,
